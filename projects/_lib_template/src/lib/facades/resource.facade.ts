...
import {ResourceRepository, Query, CreateCommand, UpdateCommand, Id} from '../ports/resource-repository.port';
...
export class ResourceFacade<T extends { id?: Id }> implements Facade<T> {
    ...
    list(query?: Query): void {
        ...
        this.repo.list(query).pipe(
            catchError(err => {
                console.error('Erreur list()', err);
                return throwError(() => err);
            }),
            finalize(() => this._loading.set(false))
        ).subscribe(page => this._items.set(page.member));
    }
